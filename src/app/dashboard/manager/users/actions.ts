...
export async function deleteSystemUser(id: string) {
  const { user } = await verifySession(['manager'])
  
  if (user.id === id) {
    return { message: 'Security Policy: You cannot recursively drop your own Manager account.', error: true }
  }
