...
export async function getStaffDetailReport(staffId: string, targetDate?: string) {
  ...
  const [
    ...
    supabase.from('sales').select('id, total_amount, sale_type, created_at, sale_items(quantity, unit_price), customer:customers(name)').eq('staff_id', staffId).order('created_at', { ascending: false })
  ])
