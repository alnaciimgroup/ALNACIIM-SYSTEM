...
export async function AccountantDashboardContent({ dateFilter, customDate }: { dateFilter?: string, customDate?: string }) {
  const { metrics, todayStats, topStaff, recentActivity } = await getAccountantOverview(dateFilter, customDate)

  const isFullyAudited = (actual: number, audited: number) => true
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <Link 
          ...
          className="bg-[#0f172a] rounded-[24px] p-6 shadow-xl flex flex-col hover:shadow-2xl transition-all duration-300 text-white group cursor-pointer lg:col-span-1"
        >
          <div className="flex items-center justify-between mb-4">
            <div className="w-12 h-12 rounded-[16px] bg-white/10 text-white flex items-center justify-center group-hover:bg-[#ef4444] transition-colors">
              <AlertCircle size={24} strokeWidth={2.5} />
            </div>
            <ArrowUpRight size={20} className="text-white/20 group-hover:text-white transition-colors" />
          </div>
          <div className="mt-auto">
            <span className="text-[11px] font-black text-white/40 uppercase tracking-widest leading-none mb-1.5 block">Total Global Debt</span>
            <div className="text-[32px] font-black text-white leading-none tracking-tighter">${metrics?.outstandingBalance?.toLocaleString() ?? 0}</div>
          </div>
        </Link>
      </div>
