...
export function RecordSaleForm({ customers, remainingStock }: { customers: Customer[], remainingStock: number }) {
  ...
  const hasDiscount = discount !== '' && !isNaN(discountParsed)
  const discountAmount = hasDiscount ? Math.max(0, discountParsed) : 0
  const finalTotal = salesType === 'free' ? 0 : Math.max(0, standardTotal - discountAmount)
    ...
    <div className="bg-white border border-[#e5e7eb] shadow-sm rounded-[24px] p-8 mb-8">
      ...
      <form action={formAction} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-2.5">
          ...
          <div className="flex gap-3 h-[50px]">
            <label className={`flex-1 flex items-center justify-center gap-1.5 lg:gap-2 rounded-[12px] border-2 cursor-pointer transition-all font-bold text-[12px] lg:text-[14px] ${salesType === 'cash' ? 'bg-[#eff6ff] border-[#3b82f6] text-[#3b82f6]' : 'bg-white border-[#e2e8f0] text-[#64748b] hover:bg-[#f8fafc]'}`}>
              <input type="radio" name="sale_type" value="cash" checked={salesType === 'cash'} onChange={() => handleSalesTypeChange('cash')} className="hidden" />
            ...
            <label className={`flex-1 flex items-center justify-center gap-1.5 lg:gap-2 rounded-[12px] border-2 cursor-pointer transition-all font-bold text-[12px] lg:text-[14px] ${salesType === 'credit' ? 'bg-[#fff7ed] border-[#f59e0b] text-[#f59e0b]' : 'bg-white border-[#e2e8f0] text-[#64748b] hover:bg-[#f8fafc]'}`}>
              <input type="radio" name="sale_type" value="credit" checked={salesType === 'credit'} onChange={() => handleSalesTypeChange('credit')} className="hidden" />
            ...
            <label className={`flex-1 flex items-center justify-center gap-1.5 lg:gap-2 rounded-[12px] border-2 cursor-pointer transition-all font-bold text-[12px] lg:text-[14px] ${salesType === 'free' ? 'bg-[#ecfdf5] border-[#10b981] text-[#10b981]' : 'bg-white border-[#e2e8f0] text-[#64748b] hover:bg-[#f8fafc]'}`}>
              <input type="radio" name="sale_type" value="free" checked={salesType === 'free'} onChange={() => handleSalesTypeChange('free')} className="hidden" />
            ...
            <label className={`flex-1 flex items-center justify-center gap-1.5 lg:gap-2 rounded-[12px] border-2 cursor-pointer transition-all font-bold text-[12px] lg:text-[14px] ${salesType === 'draft' ? 'bg-[#f8fafc] border-[#94a3b8] text-[#475569]' : 'bg-white border-[#e2e8f0] text-[#64748b] hover:bg-[#f8fafc]'}`}>
              <input type="radio" name="sale_type" value="draft" checked={salesType === 'draft'} onChange={() => handleSalesTypeChange('draft')} className="hidden" />
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="discount" className="text-[12px] font-extrabold text-[var(--foreground)] uppercase tracking-wider">Discount ($)</label>
          <div className="relative">
            <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            ...
            <input 
              ...
              step="any"
              min="0"
              max={standardTotal.toFixed(2)}
              placeholder="0.00"
              value={discount}
              onChange={(e) => setDiscount(e.target.value)}
              disabled={salesType === 'free' || qty === 0}
              className={`w-full h-[50px] pl-[44px] pr-4 bg-[var(--background)] border rounded-[12px] text-[15px] font-bold text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[#3b82f6]/20 focus:border-[#3b82f6] transition-all ${(salesType === 'free' || qty === 0) ? 'bg-gray-100 opacity-60 cursor-not-allowed' : 'border-[var(--border)]'}`}
            />
          </div>
        </div>
        ...
        {/* Agreed Final Total ($) */}
        <div className="flex flex-col gap-2">
          <label htmlFor="final_total" className="text-[12px] font-extrabold text-[var(--foreground)] uppercase tracking-wider">Agreed Final Total ($)</label>
          <div className="relative">
            <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            ...
            <input 
              ...
              step="any"
              readOnly
              value={salesType === 'free' ? '0.00' : (qty > 0 ? finalTotal.toFixed(2) : '')}
              placeholder={standardTotal.toFixed(2)}
              className="w-full h-[50px] pl-[44px] pr-4 bg-gray-100 border border-[var(--border)] rounded-[12px] text-[15px] font-bold text-[var(--muted-foreground)] opacity-70 cursor-not-allowed focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="md:col-span-2 pt-6 border-t border-[#f1f5f9] mt-2 flex flex-col gap-4">
          ...
          <button 
            type="submit" 
            disabled={isPending || isOverStock || (qty === 0 && freeQty === 0) || remainingStock <= 0 || discountAmount > standardTotal}
